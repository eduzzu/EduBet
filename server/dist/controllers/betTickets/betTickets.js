...
export const getBettingTickets = async (req, res) => {
    try {
...
export const addEventToTicket = async (req, res) => {
    try {
        const { eventId, odd, stake } = req.body;
        const userId = req.user.id;
        const ticket = await BettingTicket.findOne({ userId: userId, status: 'Unset' });
        if (ticket) {
            const existingSelectionIndex = ticket.selections.findIndex(selection => selection.eventId.equals(eventId));
            ...
            const totalOdd = ticket.selections.reduce((total, selection) => total * selection.odd, 1);
...
export const removeEventFromTicket = async (req, res) => {
    try {
        ...
        const eventIdx = ticket.selections.findIndex((selection) => selection.eventId.equals(eventId));
        if (eventIdx === -1) {
...
export const updateTicketStatus = async (req, res) => {
    try {
        ...
        const eventsLostStatus = ticket?.selections.some((selection) => selection.status === "Lost");
        const eventsWonStatus = ticket?.selections.every((selection) => selection.status === "Won");
        if (eventsLostStatus) {
            ticket.status = "Lost";
        }
...
export const updateEventStatus = async (req, res) => {
    try {
        ...
        const event = ticket.selections.find((selection) => selection.eventId.toString() === eventId);
        if (!event) {
