...
export const generateOdds = (probability: number) => {
    let odds = (1/probability).toFixed(2);
    return parseFloat(odds);
}

export const updateUserAccountBalanceAfterPlacingBet = async(ticketId: string) => {
    try{
...
export const updateUserAccountBalanceAfterWinningBettingTicket = async(ticketId: string) => {
    try{
        const ticket = await BettingTicket.findById(ticketId);
        if(!ticket) {
            throw new Error("Betting ticket not found.");
